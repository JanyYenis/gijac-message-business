...
@section('content')
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            <div>
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                    ...
                    <div class="ms-3">
                        <h1 class="text-white">
                            <i class="bi bi-diagram-3 text-white fs-1"></i>{{ __('Constructor de Chatbots') }}</h1>
            ...
            <div class="mt-3 mt-md-0">
                <button class="btn btn-light-soft d-none" id="btnImportTop">
                    <i class="bi bi-upload text-white"></i>{{ __('Importar Flujo') }}</button>
                {{-- <button class="btn btn-white" data-bs-toggle="modal" data-bs-target="#modalNuevoFlujo">
                    <i class="bi bi-plus-lg text-primary"></i>{{ __('Nuevo Flujo') }}</button> --}}
            </div>
        </div>
    </div>
        ...
        <div class="builder-grid">
            ...
            <div class="col-panel">
                <div class="panel-head">
                    <h6 class="fs-4">
                        <i class="bi bi-grid-1x2 text-dark me-1"></i>{{ __('Biblioteca de Nodos') }}</h6>
                </div>
                <div class="panel-body">
                    ...
                    <button class="cat-toggle" data-target="cat-mensajes">
                        <span>
                            <i class="bi bi-chat-square-text text-dark me-1"></i>{{ __('Mensajes') }}</span>
            ...
            <div class="col-panel" style="border-right:1px solid var(--border-soft);">
                <div class="canvas-toolbar">
                    <button class="btn btn-sm btn-wa-save btn-toolbar" id="btnPublicar"
                        ...
                        <i class="bi bi-save text-white me-1"></i>{{ __('Guardar / Publicar') }}</button>
                    {{-- <button class="btn btn-sm btn-wa-save btn-toolbar" id="btnGuardar"
                        style="background:var(--wa-green); color:#fff; border-color:var(--wa-green);">
                        <i class="bi bi-save text-white me-1"></i>{{ __('Guardar') }}</button>
                    <button class="btn btn-sm btn-toolbar" id="btnPublicar">
                        ...
                        </svg>{{ __('Publicar') }}</button> --}}
                    <button class="btn btn-sm btn-toolbar d-none" data-bs-toggle="modal" data-bs-target="#modalProbar">
                        <i class="bi bi-play-circle text-dark me-1"></i>{{ __('Probar Flujo') }}</button>
                    <button class="btn btn-sm btn-toolbar d-none" id="btnDuplicar">
                        <i class="bi bi-files text-dark me-1"></i>{{ __('Duplicar') }}</button>
                    <button class="btn btn-sm btn-toolbar" id="btnExportar">
                        <i class="fas fa-file me-1"></i>{{ __('Exportar JSON') }}</button>
                    <button class="btn btn-sm btn-toolbar" id="btnHistorial">
                        <i class="bi bi-clock-history text-dark me-1"></i>{{ __('Historial') }}</button>
            ...
            <div class="col-panel right">
                <div class="panel-head">
                    <h6 class="fs-4">
                        <i class="bi bi-sliders text-dark me-1"></i>{{ __('Propiedades') }}</h6>
                    <span class="badge bg-light text-dark" id="propBadge">—</span>
                </div>
                <div class="panel-body" id="propsPanel">
                    <div class="props-empty">
                        <i class="bi bi-hand-index-thumb"></i>{{ __('Selecciona un nodo para editar su configuración') }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="table-card">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
                <h2 class="section-title fs-1" style="font-weight:700; margin:0;">{{ __('Versiones del Flujo') }}</h2>
                <p class="fs-4" style="color:var(--text-muted); margin:2px 0 0;">{{ __('Historial de cambios y publicaciones del flujo actual.') }}</p>
            </div>
        </div>
