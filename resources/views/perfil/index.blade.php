                ...
                <div class="card-body text-center">
                    ...
                    <div class="profile-photo-container">
                        <div class="profile-photo">
                            <img src="{{ $usuario?->foto ? asset($usuario?->foto ?? '#') : 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=200&h=200&fit=crop' }}"
                                alt="{{ __('Foto de perfil') }}" id="profilePhoto">
                        ...
                        <button class="btn btn-sm btn-outline btn-outline-primary mt-3" id="changePhotoBtn">
                            <i class="bi bi-camera me-2"></i>{{ __('Cambiar Foto') }}</button>
                    </div>
                    ...
                    {{-- <div class="profile-stats">
                        <div class="stat-item">
                            ...
                            <span class="stat-label">{{ __('Consultas') }}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number">0</span>
                            <span class="stat-label">{{ __('Pacientes') }}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number">0</span>
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-shield-check me-2"></i>{{ __('Estado de la Cuenta') }}</h5>
                </div>
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs card-header-tabs mt-6" id="profileTabs" role="tablist">
                        <li class="nav-item text-primary" role="presentation">
                            <button class="nav-link active" id="personal-tab" data-bs-toggle="tab"
                                ...
                                <i class="bi bi-person me-2 text-primary"></i>{{ __('Datos Personales') }}</button>
                        </li>
                        <li class="nav-item text-primary" role="presentation">
                            <button class="nav-link" id="security-tab" data-bs-toggle="tab"
                                    data-bs-target="#security" type="button" role="tab">
                                <i class="bi bi-shield-lock me-2 text-primary"></i>{{ __('Seguridad') }}</button>
                        </li>
                        @can('factura.listado')
                            <li class="nav-item text-primary" role="presentation">
                                <button class="nav-link btnTabFacturas" id="faturas-tab" data-bs-toggle="tab"
                                        data-bs-target="#tabFacturas" type="button" role="tab">
                                    <i class="bi bi-phone me-2 text-primary"></i>{{ __('Facturas') }}</button>
                            </li>
                        @endcan
                        @if (servicioPlan('api'))
                            <li class="nav-item text-primary" role="presentation">
                                <button class="nav-link" id="btnTabApiKey" data-bs-toggle="tab"
                                    ...
                                    <i class="bi bi-phone me-2 text-primary"></i>{{ __('API Keys') }}</button>
                            </li>
                        @endif
                    </ul>
                </div>
