                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                    ...
                    <div class="ms-3">
                        <h1 class="text-white">
                            ...
                            {{ __('Asistente IA para WhatsApp') }}
                        </h1>
                        <p class="subtitle mb-0">{{ __('Configura un asistente inteligente capaz de responder automáticamente a tus clientes.') }}</p>
                    </div>
                </div>
            </div>
            <div class="mt-3 mt-md-0">
                <span class="badge-active me-1">
                    @if ($asistente?->activo)
                        <span class="dot"></span>
                    @endif
                    Estado: {{ $asistente?->activo ? __('Activo') : __('Inactivo') }}
                </span>
                ...
                <div class="panel">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2">
                        <div class="char-counter">
                            <span id="charCount">0</span> {{ __('/ 2000 caracteres') }}
                        </div>
                    </div>
                    <div class="mt-3">
                        <span class="form-label d-block mb-2 fs-5">{{ __('Ejemplos rápidos') }}</span>
                        <span class="chip"
                            data-prompt="{{ __('Eres un asistente de atención al cliente de GIJAC WEB. Responde de forma amable, clara y profesional. Resuelve dudas frecuentes y guía al cliente paso a paso.') }}">
                            <i class="bi bi-headset"></i>
                            {{ __('Atención al cliente') }}
                        </span>
                        <span class="chip"
                            data-prompt="{{ __('Eres un asesor de ventas de GIJAC WEB. Tu objetivo es presentar los servicios, destacar beneficios y motivar al cliente a solicitar una cotización.') }}">
                            <i class="bi bi-cart-check"></i>
                            {{ __('Ventas') }}
                        </span>
                        <span class="chip"
                            data-prompt="{{ __('Eres un especialista de soporte técnico de GIJAC WEB. Diagnostica problemas, ofrece soluciones paso a paso y escala a un agente humano si es necesario.') }}">
                            <i class="bi bi-tools"></i>
                            {{ __('Soporte técnico') }}
                        </span>
                        <span class="chip"
                            data-prompt="{{ __('Eres un asistente de agendamiento de GIJAC WEB. Ayuda al cliente a reservar citas, confirmar disponibilidad y enviar recordatorios.') }}">
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            <span class="badge bg-light text-dark mb-2 fs-5">
                                <i class="bi bi-hdd-network text-dark me-1 fs-5"></i>{{ __('Ollama (modelos locales, 100% gratis)') }}
                            </span>
                        </div>
                        <div class="col-md-12">
            ...
            <div class="col-12 col-lg-4">
                    ...
                    <div class="switch-list" id="capList">
                        @foreach ($capacidadOpciones as $item)
                            <div class="switch-row">
                                <span class="lbl">{{ __($item?->nombre) }}</span>
                ...
                <div class="panel">
                    ...
                    <textarea class="form-control" rows="4" placeholder="{{ __('cotización, precio, asesor, factura, soporte') }}"
                        name="palabras_clave" id="palabrasClave">{{ $asistente?->palabras_clave ? implode(', ', $asistente?->palabras_clave) : __('cotización, precio, asesor, factura, soporte') }}</textarea>
                </div>
                ...
                <div class="panel">
                    ...
                    <textarea class="form-control mb-3" rows="2" name="mensaje_bienvenida" id="mensajeBienvenida"
                        >{{ $asistente?->mensaje_bienvenida ?? __('¡Hola! Soy Gibot, tu asistente de GIJAC WEB. ¿En qué puedo ayudarte hoy?') }}</textarea>
                    ...
                    <textarea class="form-control mb-3" rows="2" name="mensaje_fuera_horario"
                        >{{ $asistente?->mensaje_fuera_horario ?? __('Gracias por escribirnos. Nuestro horario es de 8:00 a 18:00. Te responderemos pronto.') }}</textarea>
                    ...
                    <textarea class="form-control" rows="2" name="mensaje_transferencia"
                        >{{ $asistente?->mensaje_transferencia ?? __('Un momento por favor, te estoy conectando con un asesor humano.') }}</textarea>
                </div>
        ...
        <div class="sim-wrap flex-column flex-md-row">
            <div class="sim-info">
                ...
                <div class="feat fs-4">
                    ...
                    {{ __('Respuestas automáticas') }}
                </div>
                <div class="feat fs-4">
                    <i class="bi bi-check-circle-fill text-white"></i>
                    {{ __('Basado en documentos') }}
                </div>
                <div class="feat fs-4">
                    <i class="bi bi-check-circle-fill text-white"></i>
                    {{ __('Transferencia a humano') }}
                </div>
                <div class="feat fs-4">
                    <i class="bi bi-check-circle-fill text-white"></i>
                    {{ __('Memoria de conversación') }}
                </div>
            </div>
            <div class="sim-chat">
                <div class="sim-chat-head fs-4">
                    <i class="bi bi-whatsapp text-white fs-4"></i>
                    {{ __('Vista previa WhatsApp') }}
                </div>
                <div class="sim-body" id="simBody">
                    <div class="bubble in">{{ $asistente?->mensaje_bienvenida ?? __('¡Hola! Soy Gibot, tu asistente de GIJAC WEB. ¿En qué puedo ayudarte hoy?') }}
