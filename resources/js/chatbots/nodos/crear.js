...
function saveNode() {
    ...
    const message = $('#addNodeModal #nodeMessage').val().trim();
    if (!message) {
        Swal.fire('Error', 'El mensaje es obligatorio', 'error');
        return;
    }
    ...
    currentNodeId++;
    const newNode = {
        id: $('#addNodeModal #idNodo').val() ?? currentNodeId,
        type: selectedNodeType,
        message: message,
        connections: []
    };
    ...
    if (selectedNodeType === 'buttons') {
        const buttons = [];
        const buttonsNode = [];
        $('#addNodeModal .button-text').each(function() {
            const text = $(this).val().trim();
            if (text) buttons.push(text);
        });
        $('#addNodeModal .button-node').each(function() {
            const text = $(this).val().trim();
            if (text) buttonsNode.push(text);
        });
        newNode.buttons = buttons;
        ...
        const options = [];
        const listNode = [];
        $('#addNodeModal .list-text').each(function() {
            const text = $(this).val().trim();
            if (text) options.push(text);
        });
        $('#addNodeModal .list-node').each(function() {
            console.log('sin trim: '+$(this).val());
            console.log('con trim: '+$(this).val().trim());
        ...
        newNode.variable = $('#addNodeModal #nodeVariable').val().trim();
        newNode.connections.push({
            response: '*',
            nextNode: null
        });
...
function editNode(nodeId) {
    const node = currentChatbot.nodes.find(n => n.id === nodeId);
    if (!node) return;
...
function saveChatbot() {
    ...
    Swal.fire({
        ...
        inputValue: currentChatbot.name,
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            currentChatbot.nodes.forEach((node, index) => {
                if ((node.type == 'document' || node.type == 'image' || node.type == 'video') && node.archivo) {
            ...
            const ruta = route("chatbots.store");
            generalidades.create(ruta, config, success, error);
            generalidades.mostrarCargando('body');

            // if (isEditing) {
            //     // Update existing chatbot
            //     const index = chatbots.findIndex(c => c.id === currentChatbot.id);
            //     if (index !== -1) {
            //         chatbots[index] = currentChatbot;
            //     }
            // } else {
            //     // Create new chatbot
            //     currentChatbot.id = Math.max(...chatbots.map(c => c.id)) + 1;
            //     currentChatbot.createdDate = new Date().toLocaleDateString('es-ES');
            //     chatbots.push(currentChatbot);
            // }
        }
    });
}
