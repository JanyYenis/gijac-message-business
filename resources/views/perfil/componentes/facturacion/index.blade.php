    ...
    <div class="card-body">
            ...
            <div class="notice d-flex bg-light-info rounded border-info border border-dashed mb-12 p-6">
                ...
                <div class="d-flex flex-stack flex-grow-1 ">
                    ...
                    <div class=" fw-semibold">
                        ...
                        <div class="fs-6 text-gray-700 ">{{ __('El estado de tu ultima transaccion fue') }} {{ $ultimaTransaccion?->x_response }}
        ...
        @if ($ultimaFacturaPagada && $plan)
            ...
            <div class="row">
                ...
                <div class="col-lg-7">
                    <!--begin::Heading-->
                    <h3 class="mb-2">{{ __('Válido hasta el') }} {{ $ultimaFacturaPagada?->fecha_vencimiento->translatedFormat('j \d\e F \d\e\l Y') }}</h3>
                    <p class="fs-6 text-gray-600 fw-semibold mb-6 mb-lg-15">
                        {{ __('Le enviaremos una notificación al vencimiento de su suscripción.') }}</p>
                    <!--end::Heading-->

                    <!--begin::Info-->
                    <div class="fs-5 mb-2">
                        <span class="text-gray-800 fw-bold me-1">${{formatoMiles($ultimaFacturaPagada?->value)}}</span>
                        <span class="text-gray-600 fw-semibold">{{ __('por') }} {{ $ultimaFacturaPagada?->tiempo }} {{ $ultimaFacturaPagada?->tiempo == 1 ? __('mes') : __('meses') }}</span>
                    </div>
                    <!--end::Info-->
                ...
                <div class="col-lg-5">
                    ...
                    <div class="d-flex text-muted fw-bold fs-5 mb-3">
                        ...
                        @if ($plan?->max_contactos)
                            <span class="text-gray-800">{{ $cantidad_contactos_activos }} de {{ $plan?->max_contactos ?? __('Ilimitado') }} {{ __('contactos') }}</span>
                        @else
                    ...
                    <div class="progress h-8px bg-light-primary mb-2">
                        <div class="progress-bar bg-primary" role="progressbar"
                            style="width: {{ $plan?->max_contactos && $cantidad_contactos_activos ?
                            ($cantidad_contactos_activos / ($plan?->max_contactos ?? 0)) * 100 : 0 }}%"
                            aria-valuenow="{{ $plan?->max_contactos && $cantidad_contactos_activos ?
                            ($cantidad_contactos_activos / ($plan?->max_contactos ?? 0)) * 100 : 0 }}"
                    ...
                    @if ($plan?->max_contactos)
                        ...
                        <div class="fs-6 text-gray-600 fw-semibold mb-10">
                            {{ __('Quedan') }} {{ ($plan?->max_contactos ?? 0) && $cantidad_contactos_activos ? ($plan?->max_contactos ?? 0) - $cantidad_contactos_activos : 0 }} {{ __('contactos hasta que su plan requiera actualización') }}
                        </div>
                        <!--end::Description-->
                    @endif
...
<div class="card mb-5 mb-xl-10">
    ...
    <div class="card-body">
        <div class="scroll-y me-n5 pe-5 h-lg-auto tablasScroll" data-kt-scroll="true"
            data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto"
            data-kt-scroll-dependencies="#kt_header, #kt_toolbar, #kt_footer, #kt_chat_contacts_header"
            data-kt-scroll-wrappers="#kt_content, #kt_chat_contacts_body" data-kt-scroll-offset="5px" style="max-height: 410px;">
